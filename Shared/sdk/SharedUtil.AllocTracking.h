...
void* myNew(std::size_t size, const char* Tag = NULL, int ElementSize = -1);
    ...
    #endif
inline void* operator new(std::size_t size, int, const char* Tag, int ElementSize)
{
    return myNew(size, Tag, ElementSize);
}
inline void* operator new(std::size_t size, int, const char* Tag)
{
    return myNew(size, Tag);
}
inline void* operator new(std::size_t size) throw(STD_BAD_ALLOC)
{
    return myNew(size);
}
inline void* operator new[](std::size_t size, int, const char* Tag, int ElementSize) throw(STD_BAD_ALLOC)
{
    return myNew(size, Tag, ElementSize);
}
inline void* operator new[](std::size_t size, int, const char* Tag) throw(STD_BAD_ALLOC)
{
    return myNew(size, Tag);
}
inline void* operator new[](std::size_t size) throw(STD_BAD_ALLOC)
...
inline void operator delete[](void* ptr, const std::nothrow_t&) throw()
{
    ...
    #ifdef WIN32
inline void* _malloc_(size_t Count)
{
    return myMalloc(Count);
}
inline void* _calloc_(size_t Count, size_t elsize)
{
    return myCalloc(Count, elsize);
}
inline void* _realloc_(void* Original, size_t Count)
{
    return myRealloc(Original, Count);
}
inline void _free_(void* Original)
{
