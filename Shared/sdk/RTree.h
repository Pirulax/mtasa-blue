...
public:
    ...
    void Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::vector<DATATYPE>& resultList);
...
protected:
    ...
    bool         Search(Node* a_node, Rect* a_rect, std::vector<DATATYPE>& resultList);
...
bool RTREE_QUAL::Load(RTFileStream& a_stream)
{
    ...
    if ((dataFileId == _dataFileId) && (dataSize == _dataSize) && (dataNumDims == _dataNumDims) && (dataElemSize == _dataElemSize) &&
        (dataElemRealSize == _dataElemRealSize) && (dataMaxNodes == _dataMaxNodes) && (dataMinNodes == _dataMinNodes))
    {
...
void RTREE_QUAL::ChoosePartition(PartitionVars* a_parVars, int a_minFill)
{
    ...
    while (((a_parVars->m_count[0] + a_parVars->m_count[1]) < a_parVars->m_total) && (a_parVars->m_count[0] < (a_parVars->m_total - a_parVars->m_minFill)) &&
...
bool RTREE_QUAL::RemoveRect(Rect* a_rect, const DATATYPE& a_id, Node** a_root)
{
    ...
    if (!RemoveRectRec(a_rect, a_id, *a_root, &reInsertList))
    {
        ...
        while (reInsertList)
        {
            ...
            {
                InsertRect(&(tempNode->m_branch[index].m_rect), tempNode->m_branch[index].m_data, a_root, tempNode->m_level);
            }
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, std::vector<DATATYPE>& resultList)
{
    ASSERT(a_node);
    ASSERT(a_node->m_level >= 0);
    ASSERT(a_rect);
    ...
    if (a_node->IsInternalNode())            // This is an internal node in the tree
    {
        for (int index = 0; index < a_node->m_count; ++index)
        {
            if (Overlap(a_rect, &a_node->m_branch[index].m_rect))
            {
                if (!Search(a_node->m_branch[index].m_child, a_rect, resultList))
                {
