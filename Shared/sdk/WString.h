...
public:
    ...
    WString(const std::wstring& strText) : std::wstring(strText) {}
    ...
    // Access
    wchar_t& operator[](int iOffset) { return std::wstring::operator[](iOffset); }

    // Operators
    WString operator+(const wchar_t* other) const { return std::wstring(*this) + other; }
    WString operator+(const std::wstring& other) const { return std::wstring(*this) + other; }
