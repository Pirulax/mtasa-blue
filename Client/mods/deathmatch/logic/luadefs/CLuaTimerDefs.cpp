...
int CLuaTimerDefs::SetTimer(lua_State* luaVM)
{
    ...
    if (!argStream.HasErrors())
    {
        ...
        if (luaMain)
        {
            ...
            {
                CLuaTimer* pLuaTimer = luaMain->GetTimerManager()->AddTimer(iLuaFunction, CTickCount(dTimeInterval), Repeats, std::move(Arguments));
                if (pLuaTimer)
                {
...
int CLuaTimerDefs::GetTimers(lua_State* luaVM)
{
    ...
    if (!argStream.HasErrors())
    {
        ...
        if (pLuaMain)
        {
            ...
            CTickCount                            ticksNow = CTickCount::Now();
            unsigned int                          uiIndex = 0;

            for (const CLuaTimer& timer : pLuaTimerManager->GetAllTimers())
            {
                // If the time left is less than the time specified, or the time specifed is 0
                if (dTime == 0 || timer.GetTimeLeft(ticksNow).ToDouble() <= dTime)
                {
                    // Add it to the table
                    lua_pushnumber(luaVM, ++uiIndex);
                    lua_pushtimer(luaVM, &timer);
