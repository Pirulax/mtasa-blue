...
{
public:
    CClientEntityBasePtr() { pEntity = NULL; }
    ...
    ~CClientEntityBasePtr() { ChangeValue(NULL); }

    void operator=(T* pNewValue) { ChangeValue(pNewValue); }

    operator T*() { return pEntity; }

    operator const T*() const { return pEntity; }

    T* operator->() { return pEntity; }

    const T* operator->() const { return pEntity; }
