...
void CSyncDebug::OnDraw()
{
    ...
    SString strBuffer(
        ...
        m_pPlayer->GetNick(), vecPosition.fX, vecPosition.fY, vecPosition.fZ, fCurrentRotation, m_pPlayer->IsDead(), m_pPlayer->GetLatency(),
        m_pPlayer->GetPlayerSyncCount(), m_pPlayer->GetVehicleSyncCount(), m_pPlayer->GetKeySyncCount());
    ...
    g_pCore->GetGraphics()->DrawRectangle(0.75f * fResWidth, 0.30f * fResHeight, 0.25f * fResWidth, 0.50f * fResHeight, 0x78000000);
    ...
    strBuffer.Format(
        ...
        g_pNet->GetPing(), m_usFakeLagPing, m_usFakeLagVariance,

        stats.packetsReceived, stats.packetsSent, strBytesRecv.c_str(), strBytesSent.c_str(), strRecvRate.c_str(), strSendRate.c_str(),
        m_uiPacketsReceived - m_uiLastPacketsReceived, m_uiPacketsSent - m_uiLastPacketsSent);
